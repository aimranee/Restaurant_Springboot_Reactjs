...
const SerieList = () => {
    ...
    const handleDelete = (id) => {
      if (window.confirm("Are you sure you want to delete this serie ?")) {
        axios.delete(`/api/series/delete/id/${id}`).then(() => {
  ...
  return (
    <div>
       <h2>serie List</h2>
    <a class="btn btn-success" href="/addserie">Ajouter serie</a>
<table class="table table-bordered ">
  ...
  <tbody class="table-group-divider ">
            ...
            <tr key={serie.id}>
              ...
              <td>{serie.nom}</td>
              <td>
                <button type="button" class="btn btn-warning" onClick={() => handleEdit(serie.id)}>Edit</button><b>     </b>
                <button type="button" class="btn btn-danger" onClick={() => handleDelete(serie.id)}>Deleter</button>
              </td>
            </tr>
          ))}
  </tbody>
